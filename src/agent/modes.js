            ...
            if (this.stuck_time > max_stuck_time) {
                ...
                execute(this, agent, async () => {
                    ...
                    const recoveryTimeout = setTimeout(() => {
                        ...
                        console.warn('Unstuck recovery timed out. Interrupting current action instead of killing process.');
                        agent.requestInterrupt();
                    }, 10000);
                    await skills.moveAway(bot, 5);
                    clearTimeout(recoveryTimeout);
                    if (forcedInterrupt) {
                        say(agent, 'Unstuck recovery took too long, I interrupted actions and will retry.');
                    }
                    else {
                        say(agent, 'I\'m free.');
                    }
